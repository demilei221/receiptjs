...
// Import the function from the compiled JavaScript file
// const { createReceiptElement } = require('../dist/receipt-util');
...
const Receipt = (() => {
    ...
    const parseLine = (columns, state) => {
        ...
        const line = columns
            ...
            .map((column, index, array) => {
                ...
                // determin alignment from white spaces around column text
    ...
    const _element = {
        ...
        // print horizontal rule:
        hr(width) {
            return '{"Type": "TEXT", ' + '"Data": "' + ' '.repeat(this.left) + '-'.repeat(width) + '' + '", "SubType": null}';
        },
        // print vertical rules:
        vr(widths, height) {
            this.buffer.push({ data: '|', index: this.position, length: 1 });
        ...
        barcode(symbol, encoding) {
            return '{"Type": "IMAGE", "Data": "' + symbol.data + '", ' + '"SubType": "Barcode"}';
        }, 
        image(image) {
            return '{"Type": "IMAGE", "Data": "' + image + '", "SubType": null}';
        },
        ...
        lf() {
            ...
            this.buffer = [];
            // return r;
            return '{"Type": "TEXT", "Data": "' + r + '", "SubType":null}';
        }
    };
    ...
    const toElementHelper = async (markdown, printer) => {
        // initialize state variables
        const state = {
            wrap: true,
            border: 1,
        ...
        const res = [];
        for (let line of markdown.normalize().split(/\n|\r\n|\r/)) {
            let line = parseLine(line, state)
            let newline = await createLine(line, printer, state)
            res.push(newline);
        }
        // if rules is not finished
        switch (state.line) {
        ...
        from(markdown, options) {
            ...
            return {
                ...
                async toElement() {
                    ...
                    return await transform(markdown, { ...params, ...{ command: commands.element } });
